    ...
    <header className="bg-white border-b border-slate-200 text-slate-800 sticky top-0 z-40 shadow-xs">
      ...
      <div className="bg-slate-900 px-4 py-1.5 text-xs text-white border-b border-slate-800 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-slate-300">
          ...
          <span className="text-white font-medium">Usuari actual: <strong className="text-amber-300 underline">{currentUser.fullName}</strong> ({currentUser.role === 'admin' ? 'Administrador' : 'Alumne'})</span>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={onOpenRegistration}
              className="hidden sm:flex items-center gap-2 p-1.5 pl-3 rounded-full bg-slate-50 hover:bg-slate-100 border border-slate-200 text-xs font-medium transition-colors"
            >
              <div className="flex items-center gap-2">
            ...
            <button
              onClick={logout}
              className="flex items-center gap-2 px-3 py-2 rounded-lg bg-red-50 hover:bg-red-100 border border-red-200 text-red-700 text-xs font-semibold transition-colors"
              title="Cerrar sesión"
            >
